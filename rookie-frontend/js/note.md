...
运行时动态加载，可以定义变量接收，且输出的是一个拷贝，模块的变化不会对其产生影响。

nodejs支持；原生浏览器不支持，需要webpack等工具将其转换为能在浏览器中运行的代码
...
静态编译，输出的是一个引用，模块的变化会产生影响。

只能在顶层使用，不可以在函数，循环中使用。

使用可以放在import前（我觉得其实是因为编译时确定，所以隐式优化成将import提到模块开始的地方了，因为有说法是import必须放在开头）
